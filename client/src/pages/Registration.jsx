...
// AccordionSection component (remains the same)
function AccordionSection({ title, children }) {
  // ... (Your AccordionSection code here) ...
  const [isOpen, setIsOpen] = useState(false);
...
// End of AccordionSection

function Registration() {
  // --- NEW STATE for controlling view ---
  ...
  async function handleSubmit(e) {
    ...
    try {
      await axiosPublic.post("deacons/register", payload);

      // On success, show the Thank You page
      setIsSubmitted(true);
    } catch (err) {
      console.error(err);
      alert(err.response?.data?.message || "Registration failed");
  ...
  // --- The original Form Rendered when isSubmitted is false ---
  return (
    <div>
      <section className="min-h-screen bg-gray-100 py-20 px-5 md:px-10">
        {/* Hero Header */}
        <div className="text-center px-5 mb-16 shadow-lg bg-white/90 py-8 rounded-2xl max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-extrabold text-brand-primary">
            ተፈሣህኩ እስመ ይቤሉኒ ቤተ-<span className="text-red-500">-እግዚአብሔር</span>{" "}
            ነሐውር፡፡
          </h1>
          <p className="text-lg text-brand-primary/80 md:text-xl mt-3 font-medium">
            ኢትዝክር ለነ አበሳነ ዘትካት ፣ ፍጡነ ይርከበነ ሣህልከ እግዚኦ ፣ እስመ ተመንደብነ ፈድፋደ።
        ...
        <form className="max-w-lg mx-auto" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="w-full py-4 mt-6 bg-brand-secondary text-brand-primary font-extrabold text-xl rounded-full shadow-2xl hover:scale-[1.02] transition duration-300 cursor-pointer"
          >
            Submit Application
          </button>
        </form>
